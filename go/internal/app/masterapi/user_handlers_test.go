...
func testUserReadServer(t *testing.T) (*Server, *sql.DB) {
	...
	statements := []string{
		...
		`ALTER TABLE users ADD COLUMN service_id INTEGER NULL`,
		...
		`ALTER TABLE nodes ADD COLUMN xray_config_mode TEXT DEFAULT 'remote'`,
		`ALTER TABLE nodes ADD COLUMN xray_config TEXT NULL`,
		...
		`CREATE TABLE hosts (
			...
			mux_enable INTEGER DEFAULT 0,
			fragment_setting TEXT NULL,
			noise_setting TEXT NULL,
			random_user_agent INTEGER DEFAULT 0,
			use_sni_as_host INTEGER DEFAULT 0
		)`,
		`CREATE TABLE service_hosts (service_id INTEGER, host_id INTEGER, sort BIGINT DEFAULT 0)`,
